...
public class UserController {
    ...
    // 내 정보 조회
    @GetMapping("/me")
    public ResponseEntity<ApiResponse<User>> getMyInfo(@AuthenticationPrincipal UserDetails userDetails) {
        // userDetails.getUsername() == email
        User user = userService.getUserInfoByEmail(userDetails.getUsername());
        return ResponseEntity.ok(ApiResponse.success(user, "내 정보 조회 성공"));
    }

    // 내 정보 수정
    @PutMapping("/me")
    public ResponseEntity<ApiResponse<User>> updateMyInfo(@AuthenticationPrincipal UserDetails userDetails,
                                                          @RequestBody UpdateUserInfoRequest request) {
        User user = userService.updateUserInfoByEmail(userDetails.getUsername(), request.getNickname(), request.getPhoneNumber());
    ...
    public ResponseEntity<ApiResponse<Void>> changePassword(@AuthenticationPrincipal UserDetails userDetails,
        ...
        userService.changePasswordByEmail(userDetails.getUsername(), request.getCurrentPassword(), request.getNewPassword());
